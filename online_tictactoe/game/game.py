...
from online_tictactoe.schemas.board import Board
from online_tictactoe.schemas.cell_state import CellState
from online_tictactoe.error import NoWinnersFound, WinnerTypeNotImplemented
...
class Game:
    ...
    def take_turn(self, player: Player):
        cell = int(
            input(
                f"{player.name} Which cell do you want to play in (0 is the top left 3 the middle left and 8 the bottom right):"
            )
        )
    ...
    def winner(self) -> Player:
        if not self.is_over:
            raise NoWinnersFound("Calling .winner attr when game is not over")
        ...
        if self.winner_type == WinnerType.PLAYER1:
            ...
            raise WinnerTypeNotImplemented(
                f"Winner type {self.winner_type} not implemented"
            )
    ...
    def winner_type(self) -> WinnerType:
        for i in range(len(self.board.board)):
            if (
                self.board.get_cell_state(i, 0)
                == self.board.get_cell_state(i, 1)
                == self.board.get_cell_state(i, 2)
                != CellState.EMPTY
            ):
                return (
                    WinnerType.PLAYER1
                    if self.board.get_cell_state(i, 0) == CellState.PLAYER1
                    else WinnerType.PLAYER2
                )

        for i in range(len(self.board.board)):
            if (
                self.board.get_cell_state(0, i)
                == self.board.get_cell_state(1, i)
                == self.board.get_cell_state(2, i)
                != CellState.EMPTY
            ):
                return (
                    WinnerType.PLAYER1
                    if self.board.get_cell_state(0, i) == CellState.PLAYER1
                    else WinnerType.PLAYER2
                )

        if (
            self.board.get_cell_state(0, 0)
            == self.board.get_cell_state(1, 1)
            == self.board.get_cell_state(2, 2)
            != CellState.EMPTY
        ):
            return (
                WinnerType.PLAYER1
                if self.board.get_cell_state(0, 0) == CellState.PLAYER1
                else WinnerType.PLAYER2
            )

        if (
            self.board.get_cell_state(0, 2)
            == self.board.get_cell_state(1, 1)
            == self.board.get_cell_state(2, 0)
            != CellState.EMPTY
        ):
            return (
                WinnerType.PLAYER1
                if self.board.get_cell_state(0, 2) == CellState.PLAYER1
                else WinnerType.PLAYER2
            )
    ...
    def is_board_full(self) -> bool:
        return all(
            cell_state is not CellState.EMPTY
